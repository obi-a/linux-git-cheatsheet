...
- **`cd`**: changes to the specified absolute or relative path. For example  
...
- **`rm`**: removes a file, list of files or directories.
...
- **`cp`**: copies source file to the destination.
  + `cp <source-file> <destination-file>`: copies source file to the destination-file. 
- **`pwd`**: displays the path of the current working directory. 
- **`touch`**: creates a single or multiple new files with the specified name(s).
  + `touch <filename>`: creates the specified file. 
  + `touch <file1> <file2> <file3>`: creates the specified files.
- **`cat`**: displays the contents of the specified file or files on the screen.
...
## Git Commands
- **`git add`**: stages a file or group of files. 
...
- **`git pull`**: pulls changes from remote branch into the current local branch. 
- **`git branch`**: lists all local branches while indicating the current branch.
- **`git checkout`**: switches to specified branch, or creates a new branch.
